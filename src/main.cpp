...
bool loadConfigSPIFFS() {
    if(!SPIFFS.begin(true)) { TIMED_PRINTLN("SPIFFS Mount Failed"); return false; }
    ...
    if(!file) { TIMED_PRINTLN("Failed to open config file"); return false; }
    StaticJsonDocument<512> doc;
    DeserializationError error = deserializeJson(doc, file);
    file.close();
    if(error) { TIMED_PRINTLN("Failed to parse config file"); return false; }
...
bool saveConfigSPIFFS() {
    ...
    if(!file) { TIMED_PRINTLN("Failed to open config file for writing"); return false; }
    if(serializeJson(doc, file) == 0) { TIMED_PRINTLN("Failed to write to file"); file.close(); return false; }
    file.close();
...
void checkAndEstablishWebSocket() {
    ...
    TIMED_PRINT("mDNS query found "); Serial.println(nServices);
    ...
    for(int i = 0; i < nServices; ++i) {
        ...
        if(isValidShellyHostname(host)) {
            ...
            Serial.print("  "); Serial.print(host); Serial.print(" at "); Serial.println(ip);
        }
    }
...
void setup() {
    ...
    if(WiFi.status() != WL_CONNECTED) {
        ...
        if(WiFi.smartConfigDone()) {
            ...
            TIMED_PRINTLN("WiFi Connected.");
            TIMED_PRINT("IP Address: "); TIMED_PRINTLN(WiFi.localIP());
    ...
    if(WiFi.status() == WL_CONNECTED) {
        ...
        TIMED_PRINT("IP: "); TIMED_PRINTLN(WiFi.localIP());
        ...
        TIMED_PRINT("mDNS query found "); Serial.println(n);
